...
Если вы разрабатываете сайты, есть большой шанс на то, что хотя бы один из ваших
клиентов заказывал вам или интересовался, можете ли вы сделать сайт, который
хорошо бы отображался на мобильных устройствах. Если вы идете по пути разработки
...
контейнера, очень просто. В своей таблице стилей (или в файле стилей
нормализации/сброса стилей браузера) просто добавляете следующий код:

```img {
    max-width: 100%;
}```

В большинстве случаев это маленькое правило сделает все как надо. Когда такое
правило есть в таблице стилей, то, если контейнер изображения станет уже ширины
изображения, то изображение уменьшится в масштабе, чтобы соответствовать ширине
контейнера. Установка `max-width` на `100%` также *гарантирует, что изображение
не будет увеличиваться больше своего реального размера* (в этом случае качество
картинки очень упадет). Если вам все еще приходится поддерживать IE6/7, то
добавьте правило `width: 100%` для этих браузеров, поскольку они не понимают
`max-width`.

Это базовое решение усложняется, когда вам нужно принимать в расчет
...
делаете версию логотипа размером 300х300 и подключаете ее: все стало огромным!
...
мило масштабироваться вместе с другими изображениями на странице. Для того,
чтобы ограничить максимальную длину адаптивного изображения, нужно *ограничить
максимальную длину контейнера изображения*, а не самого изображения. Скажем, ваш
логотип находится внутри модуля, у которого стоит класс `branding`. В таком
...
Итак, у нас есть масштабируемые отзывчивые изображения в гибкой разметке нашего
сайта. Миссия выполнена. Пора посмотреть, что находится в том странном месте,
которое другие люди называют «природой» — вдруг там будет что-нибудь интересное
для зачахшего без солнца разработчика, правильно?

Не так быстро! У нас есть еще две главных проблемы, которые нужно преодолеть.
...
делают проблему еще хуже: вы не хотите посылать большую ретина-картинку на
...
десктопа. Конечно, посетитель может использовать вай-фай-соединение из местной
...
изображения и показывали маленькие на маленькой ширине, и наоборот — на большой.
...
выдать насколько возможно маленькое время загрузки, *браузеры предзагружают все
...
Из-за этой предзагрузки большинству решению нужно либо решение на бэк-энде
...
до того, как большее количество устройств не начнет ее поддерживать. [Измерять
...
## Проблема с контролем впечатления

Скажем, у вас на домашней страничке есть красивая широкая картинка. На ней —
буколические просторы полей и леса, голубое небо, пушистые облака наверху, и
счастливая семья устраивает пикник на траве.

Теперь уменьшим ее до 300 пикселей по ширине для мобильных устройств. На этом
размере наше семейство больше похоже на муравьев на пикнике.
...
изображения просто не могут хорошо масштабироваться: тонкие детали теряются, и
эмоциональное впечатление уменьшается. В случае нашего изображения, было бы
гораздо лучше, чтобы на мобильной версии изображения фотография была бы
...
ситуаций — или контролировать изображение на лету.
...
которой их можно отслеживать, хотя некоторые из совсем новых решений здесь не
учтены).

Вот несколько факторов, которые нужно иметь в виду:

* Нужно ли вам решать проблему с контролем впечатления (т.е. разные пропорции
изображения, разные варианты масштабирования и обрезки для разных размеров экрана)?
...
своей надежностью. У вас есть ваше любимое решение, которого нет в списке? Дайте
нам знать в комментариях!

## Проверенные опытом решения для отзывчивых изображений
...
специальой разметки*, в которой дивы представляют собой варианты изображений,
...
спецальную разметку: атрибуты `data-1x` и `data-2x`, которые указывают
...
У HiSRC есть и слабости: для его работы требуется jQuery, так что его имеет
...
Инструкции по установке Adaptive Images довольно многословные — пожалуй, они
выходят за рамки настоящей статьи. Если хотите узнать об этом больше, зайдите на
официальный сайт и нажмите на ссылку «Детали» наверху.

Лучшее в Adaptive Images — то, что он *не требует никакой специальной разметки
...
вашего сайта или быть недоступным на сервере вашего хостинг-провайдера. * Он
работает благодаря способности веб-сервера перехватывать запросы к изображениям
(через файл `.htaccess`). Так что если ваши изображения хранятся в каком-то
другом месте (например, на CDN), ничего небудет работать. * Этот скрипт не
определяет пропускную способность сети. * Он не предназначен для того, чтобы
контролировать впечатления от изображений: все, что он делает — это масштабирует
картинки. Он не может обрезать или делать версии изображений в других
пропорциях.

Вы, вероятно, заметили, что все решения до сих требовали JavaScript, а некоторые
еще и достаточно серьезной настройки. Если вы ищете такое решение, которому не
требуется JavaScript и которое достаточно просто внедрите, посмотрите на
следующее.
...
По умолчанию Sencha.io изменит размер вашего изображения, чтобы оно соответствовало ширине экрана устройства, которое Sencha.io определит по строке `user-agent`. Телефонам отправится изображение на 320 пикселей в ширину, планшетам — 768
пикселей в ширину и т.п.

Вы можете настроить строку префикса Sencha.io, чтобы вам возвращались конкретные
измерения, ориентации, размер в процентах — можно использовать самые сложные
вычисления. Sencha.io - бесплатный сервис для индивидуальных пользователей и
может быть очень удобным решением для адаптивных изображений. Однако вы
добавляете зависимость от стороннего сервиса, который всегда может зависнуть, и
у которого могут быть проблемы, которые вы никак не можете контролировать.
Тщательно взвесьте риски.

## Решения для отзывчивых изображений, на которые стоит обратить внимание

Те решения, которые освещены выше, можно использовать прямо сейчас, но ими дело
не ограничивается. Есть еще несколько новых многообещающих решений, и я смотрю
на них, чтобы понять, как они будут эволюционировать вместе с современным
состоянием веб-технологий. Вот два, на которые особенно интересно посмотреть.
...
неоднозначная репутация в кругах по производительности в вебе. Если ее
использовать неправильно, она может не решить проблемы с производительностью на
сайте, а создать новые!
...
стороне. Я буду еще смотреть на это решение, может быть, потом, когда IE8 и 9
...
похоже на Sencha.io Src, но в ReSRC добаляются фильтры изображений, определение
...
производитлеьность сайта. YSlow доступен для Firefox, Chrome, Safari и Opera (ну
